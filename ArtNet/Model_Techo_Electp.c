...
pModel getModelTecho_Electp(void)
{
	return (pModel)&model;
}
...
//	for(i = 0;i<=19;i++){
//		diamondRGB[i][i] = paint.background.rgb;
//		diamondRGB[0][i] = paint.background.rgb;
//		diamondRGB[i][0] = paint.background.rgb;
//		diamondRGB[i][19-i] = paint.background.rgb;
//	}
	
//	calculateParticle(&particle[0]);
//	drawDiamondCircle(particle[0].x,particle[0].y,particle[0].size);
	for(i=0;i<1;i++){
...
static void calculateParticle(pParticle particle)
{
	...
	// 边界检测
//	if(particle->x-particle->size <=0|| particle->x+particle->size >=19){
//		// 在X方向速度取反
//		particle->vx = -particle->vx;
//	}	
//	if(particle->y-particle->size <=0 || particle->y+particle->size >=19){
//		particle->vy = -particle->vy;
//	}
//	if(particle->x-particle->size <0)particle->x = particle->size;
//	if(particle->x+particle->size >19)particle->x = 19-particle->size;
//	if(particle->y-particle->size <0)particle->y = particle->size;
//	if(particle->y+particle->size >19)particle->y = 19-particle->size;
